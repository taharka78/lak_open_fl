...
Void GraphicsRenderer_obj::bindTexture( ::lime::graphics::GLRenderContext gl,::openfl::_internal::renderer::opengl::utils::GLBucket bucket){
{
		...
		if ((tmp)){
			HX_STACK_LINE(1152)
			::lime::graphics::Image tmp2 = bucket->bitmap->image;		HX_STACK_VAR(tmp2,"tmp2");
			HX_STACK_LINE(1152)
			::lime::graphics::Image tmp3 = tmp2;		HX_STACK_VAR(tmp3,"tmp3");
			HX_STACK_LINE(1152)
			tmp1 = tmp3->get_powerOfTwo();
		}
		else{
			HX_STACK_LINE(1152)
			tmp1 = false;
		...
		HX_STACK_LINE(1160)
		bool tmp2 = bucket->textureSmooth;		HX_STACK_VAR(tmp2,"tmp2");
		HX_STACK_LINE(1160)
		if ((tmp2)){
			HX_STACK_LINE(1161)
