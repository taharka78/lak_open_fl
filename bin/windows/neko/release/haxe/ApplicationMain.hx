...
@:dox(hide) class ApplicationMain {
	...
	public static function main () {
		...
		#if (js && html5)
		#if (munit || utest)
		lime.system.System.embed (projectName, null, 1024, 768, config);
		#end
		#else
		create (config);
		#end
	...
	public static function _embed (element:Dynamic, width:Null<Int> = null, height:Null<Int> = null, background:String = null, assetsPrefix:String = null) {
		...
		lime.system.System.embed ("lakhaxe", element, width, height, { background: background, assetsPrefix: assetsPrefix });
	...
	macro public static function getEntryPoint () {
		...
		switch (Context.follow (Context.getType ("Main"))) {
			...
			case TInst (t, params):
				...
				for (method in type.statics.get_()) {
...
class DocumentClass {
	...
	macro public static function build ():Array<Field> {
		...
		var classType = Context.getLocalClass ().get_();
		...
		while (searchTypes != null) {
			...
			if (searchTypes.superClass != null) {
				...
				searchTypes = searchTypes.superClass.t.get_();
