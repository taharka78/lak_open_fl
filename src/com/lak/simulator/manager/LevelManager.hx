...
class LevelManager
{
	...
	public function get9Nodes(_unit:IsoUnit):Void{
		getUnitAdjacentNodes(_unit,mapPartArray("part_"+(Math.floor(_unit.x/(IsoWorld.instance.tileW*100)))+"-"+(Math.floor(_unit.y/(IsoWorld.instance.halfH*100))+".txt")));
	}
	/*
	...
	private function getUnitAdjacentNodes(_unit:IsoUnit,tempMapAr:Array<Dynamic>):Void{		
		...
		for (i in 0...IsoUtils.spiralWalkStepArray.length){
			...
			if((temp_pt.x > -1 && temp_pt.x < 100) && (temp_pt.y > -1  && temp_pt.y < 100)){
				...
				var pos:Point = IsoUtils.mapTilePosition(temp_pt,IsoWorld.instance.halfW,IsoWorld.instance.tileH);
				var n:Dynamic = {
					...
					"g":0,
					"h":0,
					"direction":IsoUtils.spiralWalkStepArray[i]
				};
				//trace(_unit.lastNode, Std.string(n.position.x + "-" + n.position.y));
				if(_unit.lastNode != Std.string(n.position.x+"-"+n.position.y)){
