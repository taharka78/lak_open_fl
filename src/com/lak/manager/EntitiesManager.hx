...
class EntitiesManager
{
	...
	public function updateViewBounds():Void{
		screen.width = world.viewWidth;
		screen.height = world.viewHeight;
		screen.x = world.worldColOffset;
		screen.y = world.worldRowOffset;
	}
	/*
		...
		if(onScreen(pt)){
			if (!obj.clone){
				if(!obj.visible && !world.contains(obj)){ 
					...
					pt = IsoUtils.getTileAt(obj.x,obj.y,true);
